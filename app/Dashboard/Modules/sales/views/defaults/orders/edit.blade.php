...
@section('modal-body')
    ...
    <div class="row">  
            ...
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                <table id="lineitems" >
                    ...
                    @foreach( $record->orderItems() as $item )
                        <tr>
                            <td>
                                {{ Former::select('_order_product[product_id][]')->class('form-control input')->options($record->productList())->value($item->pivot->product_id) }}
                            </td>
                            <td>
                                {{ Former::select('_order_product[variant][]')->class('form-control input')->options($config['seasons'])->value($item->pivot->variant)}}
                            </td>
                            <td>
                                {{ Former::text('_order_product[quantity][]')->class('form-control input')->value($item->pivot->quantity) }}
                            </td>
                            <td>
                                {{ Former::text('_order_product[price][]')->class('form-control input')->value($record->toGBP($item->pivot->price)) }}
                            </td>
                        </tr>
                    @endforeach
        ...
        @show
            ...
            <input type="hidden" class="" name="contact_id" value="{{ $record->contact_id }}">
